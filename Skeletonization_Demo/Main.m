...
pathl = 'D:\Programs\Example\';
name_file = (strcat(pathl,'matlab.mat'));
...
for x = 10:totalF
    ...
    try
        ...
        [PXIG,skl,bds,Tworms,lenght,widths,colors] = find_state1(path,name,start_tracking,MaskBMP,worm_color,Tworms,tb_box,rs,Minimum_length-5); %get worm model
    catch
        Tworms=0;
    end
    ...
    if(Tworms>0)
        ...
        args1 = {path,name,Tworms,tb_box,worm_color,start_tracking,MaskBMP,TRuido,tt,x,totalF};%image elements
        args2 = {lenght,widths,colors,bds,PXIG,skl,rs,er}; %image elements
        ...
        [lgi,lenght,widths,colors,skl,bds,PXIG,er,colns] = predict1(args1,args2,argsC,graph,auto);%worm skeleton prediction
        clear args1; clear args2;

        %% Check and fix all lengths
        disp('Check length and segmentation...');
        color_C =35:1:42;
        args3 = {path,name,Tworms,tb_box,MaskBMP,TRuido,tt,x,totalF};%image elements
        args4 = {color_C,lgi,lenght,widths,colors,bds,PXIG,skl,rs};%image elements
        [lgi,skl,lenght,widths,colors] = check_worms(args3,args4,argsC); %Check and fix all lengths
        clear args3; clear args4;

        %% Obtain track models
        [LG,SKG,WG,CG] = get_model(lenght,skl,widths,colors,er);
        ...
        save_XML_dot(path,x,skl1,skl_status,Behavior,status,lgi2,pdt,rs,WG,CG,colns,auto);
    end
end